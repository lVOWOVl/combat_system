...
using CollectionExtensions = Edgar.Legacy.GeneralAlgorithms.Algorithms.Common.CollectionExtensions;
    ...
    public class BreadthFirstChainDecomposition<TNode> : ChainDecompositionBase<TNode>
    {
        ...
        public override List<Chain<TNode>> GetChains(IGraph<TNode> graph)
        {
            ...
            if (Faces.Count != 0)
            {
                ...
                Faces.RemoveAt(CollectionExtensions.MaxBy(Faces, x => x.Count));
            }
        ...
        private PartialDecomposition GetFirstComponent(PartialDecomposition decomposition)
        {
            ...
            if (Faces.Count != 0)
            {
                ...
                {
                    var smallestFaceIndex = CollectionExtensions.MinBy(faces, x => x.Count);
                    firstFace = faces[smallestFaceIndex];
                }
                else
                {
                    var largestFaceIndex = CollectionExtensions.MaxBy(faces, x => x.Count);
        ...
        private PartialDecomposition ExtendDecomposition(PartialDecomposition decomposition)
        {
            ...
            if (cycleComponents.Count != 0)
            {
                var nextCycleIndex = preferSmallCycles
                    ? CollectionExtensions.MinBy(cycleComponents, x => x.Nodes.Count)
                    : CollectionExtensions.MaxBy(cycleComponents, x => x.Nodes.Count);
        ...
        private GraphComponent GetCycleComponent(PartialDecomposition decomposition, List<TNode> face)
        {
            ...
            {
                var nodeIndex = CollectionExtensions.MinBy(
notCoveredNodes
, x => Graph
                            .GetNeighbors(x)
                            .Min(y =>
